...
class Orb extends Enemy {
    ...
    constructor(engine, player, type, movementType, movementArgs, powerUp = false) {
        ...
        this.shooting = new OrbShooting(this.engineRef, this, this.shootingArgs);
    ...
    #setWorker() {
        this.worker = new Worker('/wp-content/themes/SpaceForceGame/objects/enemies/orb/wOrb.js');
    ...
    #selectType(type) {
        switch (type) {
            case 1:
                this.maxHp = 250;
                this.damage = 10;
                this.shootingArgs = {bulletsOnRound: 1, speed: 125, fireRate: 2.5, gapBetweenRounds: 0.75}
                this.image.classList.add('image-1');
                break;
            case 2:
                this.maxHp = 500;
                this.damage = 20;
                this.shootingArgs = {bulletsOnRound: 2, speed: 125, fireRate: 3, gapBetweenRounds: 0.75}
                this.image.classList.add('image-2');
                break;
            case 3:
                this.maxHp = 750;
                this.damage = 30;
                this.shootingArgs = {bulletsOnRound: 2, speed: 125, fireRate: 2.75, gapBetweenRounds: 0.75}
                this.image.classList.add('image-3');
                break;
            case 4:
                this.maxHp = 1000;
                this.damage = 40;
                this.shootingArgs = {bulletsOnRound: 3, speed: 125, fireRate: 3.5, gapBetweenRounds: 0.75}
    ...
    update(data) {
        ...
        this.objectTag.style.transform = `translate(${this.x}px, ${this.y}px) rotate(${this.rotate}deg)`;
        if (data.cannon) {
            this.shooting.sides.forEach(side => {
                const dataCannon = data.cannon[side];
                if (dataCannon.length > 0) {
                    const bullets = this.shooting.existingBullets[side];
    ...
    #detectEnemyHitPlayer(playerHitboxes, enemyExistingBullets) {
        ...
        for (let i = 0; i < playerHitboxes.length; i++) {
            playerHitbox = playerHitboxes[i].getBoundingClientRect();
            this.shooting.sides.forEach(side => {
                for (let j = 0; j < enemyExistingBullets[side].length; j++) {
                    enemyBullet = enemyExistingBullets[side][j].getBoundingClientRect();
                    if (
                        ...
                        if (Player.currentHp > 0) {
                            ...
                            Player.updateHpBar(Player.currentHp);
                            this.board.removeChild(enemyExistingBullets[side][j]);
                            this.shooting.existingBullets[side].splice(j, 1);
                            if (Player.currentHp <= 0) {
                                Player.deletePlayer(this.player);
                            }
                            break;
    ...
    #deleteEnemy(byPlayer) {
        if (byPlayer) {
            ...
            if (this.player.checkPlayerHpPowerUp()) {
                ...
                if (!this.powerUp) {
                Currency.createStarsGroup('orb', this.type, this.objectTag);
                } else {
    ...
    deleteAllEnemyBullets() {
        this.shooting.sides.forEach(side => {
            this.shooting.existingBullets[side].forEach(bullet => {
                this.board.removeChild(bullet);
            });
            this.shooting.existingBullets[side] = [];
        });
    }
}