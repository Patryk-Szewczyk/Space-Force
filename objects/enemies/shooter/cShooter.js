...
class Shooter extends Enemy {
    ...
    constructor(engine, player, type, movementType, movementArgs, powerUp = false) {
        ...
        this.shooting = new ShooterShooting(this.engineRef, this, this.shootingArgs);
    ...
    #setWorker() {
        this.worker = new Worker('/wp-content/themes/SpaceForceGame/objects/enemies/shooter/wShooter.js');
    ...
    #createObject() {
        ...
        object.setAttribute('class', 'shooter');
        for (let i = 0; i < 5; i++) {
    ...
    #selectType(type) {  // OK
        switch (type) {
            case 1:
              this.maxHp = 200;
              this.damage = 15;
              this.shootingArgs = {bulletsOnRound: 1, speed: 300, fireRate: 1.5, gapBetweenRounds: 0.2};
              this.image.classList.add('image-1');
              break;
            case 2:
              this.maxHp = 350;
              this.damage = 25;
              this.shootingArgs = {bulletsOnRound: 2, speed: 300, fireRate: 1.5, gapBetweenRounds: 0.2};
              this.image.classList.add('image-2');
              break;
            case 3:
              this.maxHp = 500;
              this.damage = 35;
              this.shootingArgs = {bulletsOnRound: 2, speed: 300, fireRate: 1.275, gapBetweenRounds: 0.2};
              this.image.classList.add('image-3');
              break;
            case 4:
              this.maxHp = 650;
              this.damage = 45;
              this.shootingArgs = {bulletsOnRound: 3, speed: 300, fireRate: 1.05, gapBetweenRounds: 0.2};
    ...
    #deleteEnemy(byPlayer) {
        if (byPlayer) {
            ...
            if (this.player.checkPlayerHpPowerUp()) {
                ...
                if (!this.powerUp) {
                Currency.createStarsGroup('shooter', this.type, this.objectTag);
                } else {
