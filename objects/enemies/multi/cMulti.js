...
class Multi extends Enemy {
    ...
    constructor(engine, player, type, movementType, movementArgs, powerUp = false) {
        ...
        this.shooting = new MultiShooting(this.engineRef, this, this.shootingArgs);
    ...
    #setWorker() {
        this.worker = new Worker('/wp-content/themes/SpaceForceGame/objects/enemies/multi/wMulti.js');
    ...
    #createObject() {
        ...
        object.setAttribute('class', 'multi');
        for (let i = 0; i < 2; i++) {
    ...
    #selectType(type) {
        switch (type) {
            case 1:
                this.maxHp = 175;
                this.damage = 10;
                this.shootingArgs = {bulletsOnRound: 1, speed: 300, fireRate: 3, gapBetweenRounds: 0.4};
                this.image.classList.add('image-1');
                break;
            case 2:
                this.maxHp = 300;
                this.damage = 18;
                this.shootingArgs = {bulletsOnRound: 2, speed: 300, fireRate: 3, gapBetweenRounds: 0.4};
                this.image.classList.add('image-2');
                break;
            case 3:
                this.maxHp = 425;
                this.damage = 26;
                this.shootingArgs = {bulletsOnRound: 2, speed: 300, fireRate: 2, gapBetweenRounds: 0.6};
                this.image.classList.add('image-3');
                break;
            case 4:
                this.maxHp = 550;
                this.damage = 32;
                this.shootingArgs = {bulletsOnRound: 3, speed: 250, fireRate: 3, gapBetweenRounds: 0.75};
    ...
    #deleteEnemy(byPlayer) {
        if (byPlayer) {
            ...
            if (this.player.checkPlayerHpPowerUp()) {
                ...
                if (!this.powerUp) {
                Currency.createStarsGroup('multi', this.type, this.objectTag);
                } else {
